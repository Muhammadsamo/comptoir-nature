import { createError, useBase, createRouter, defineEventHandler } from 'h3';
...
const require$$0$3 = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(process$2);
...
const require$$0$2 = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(crypto);
...
const readPortfolio = async () => {
  try {
    const portfolio = await read("tblPortfolio");
    return {
      portfolio
    };
  } catch (error) {
...
const readProjects = async () => {
  try {
    const projects = await read("tblProjects");
    return {
      projects
    };
  } catch (error) {
...
const readServices = async () => {
  try {
    const services = await read("tblServices");
    return {
      services
    };
  } catch (error) {
...
router.get("/portfolio", defineEventHandler(readPortfolio));
router.get("/projects", defineEventHandler(readProjects));
router.get("/services", defineEventHandler(readServices));
